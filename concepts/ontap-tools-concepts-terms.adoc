...
新的 NetApp ASA r2 系统提供统一的硬件和软件解决方案，可为仅限 SAN 的客户提供特定的简化体验 https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["了解ASA R2存储系统"^]。
...
一致性组是作为一个单位管理的卷的集合。在 ONTAP 中，一致性组为跨多个卷的应用程序工作负载提供轻松管理和保护保证。详细了解 https://docs.netapp.com/us-en/ontap/consistency-groups/index.html["一致性组"^]。
...
SnapMirror 主动同步使业务服务即使在整个站点发生故障时也能继续运行，支持应用程序使用辅助副本透明地进行故障切换。使用 SnapMirror 主动同步触发故障切换无需手动干预或自定义脚本。详细了解 https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/index.html["SnapMirror活动同步"^]。
...
vSphere Metro Storage Cluster (vMSC) 是一种支持 vSphere 扩展群集部署的技术。vMSC 解决方案支持 NetApp MetroCluster 和 SnapMirror 主动同步（以前称为 SMBC）。这些解决方案可在域出现故障时提供增强的业务连续性。弹性模型基于您的特定配置选择。详细了解 https://core.vmware.com/resource/vmware-vsphere-metro-storage-cluster-vmsc["VMware vSphere Metro Storage Cluster"^]。
